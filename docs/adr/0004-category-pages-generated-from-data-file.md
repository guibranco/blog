...
This further executes the alternative flagged in [ADR-0001](0001-stub-files-for-category-tag-feed-pages.md), which is now fully superseded for pages — only its RSS-feed portion (`feed/*.xml`) still applies; feeds remain committed files, out of scope for this change.
...
- `create_missing_pages.py` no longer writes `categorias/**/*.md` files. `sync_categories_data` (already existing, now extended for `redirect_from`) is the only mechanism that keeps `_data/categories.yml` in sync with what posts declare; `create_pages` now only handles RSS feed files.
