...
| Custo de vida Dublin × São Paulo | `calculators/cost-of-living.html` | `_data/calculators/cost-of-living.yml` + `shared/fx-eur-brl.yml` | Aluguel, contas, mercado, transporte, plano de saúde, creche e lazer de uma pessoa nas duas cidades, pré-preenchidos com referências datadas e com fonte (quarto dividido ou apartamento só seu, à escolha) e todos editáveis; o que sobra do líquido em cada cidade e o líquido que a outra precisaria pagar para sobrar o mesmo, ao câmbio informado ([ADR-0019](docs/adr/0019-cost-of-living-calculator-seeds-a-sourced-basket-and-compares-what-is-left.md)) |
| Conversor de moeda do post | `calculators/fx-converter.html` | `_data/calculators/fx-converter.yml` (só textos e valores de referência) + `fx:` no front matter do post | Converte entre a moeda local da viagem e o euro na taxa da época (`fx: { from: AED, to: EUR, rate: 3.97, as_of: 2022-04 }` — `rate` é quantos `from` um `to` comprava); campos ligados nos dois sentidos, campo opcional para a cotação de hoje digitada pelo leitor (nada é buscado), data em destaque; não renderiza nada sem `fx` ([ADR-0018](docs/adr/0018-per-post-currency-converter-takes-its-rate-from-front-matter.md)) |
